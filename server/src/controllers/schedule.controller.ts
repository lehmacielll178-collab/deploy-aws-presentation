...
export class ScheduleController {
  ...
  async bookSchedule(
    ...
    try {
      const result = await scheduleService.bookSchedule(request.params.id);
      return reply.status(200).send(result);
    } catch (error) {
      if (error instanceof Error) {
        const status = error.message === 'Horário não encontrado' ? 404 : 409;
        return reply.status(status).send({ error: error.message });
      }
      return reply.status(500).send({ error: 'Erro ao agendar horário' });
    }
  }
